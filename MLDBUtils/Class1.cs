...
namespace MLDBUtils
{
	...
	public class SQLCom
	{
        ...
        {
            string toReplace=mConStr.Substring(mConStr.IndexOf("source="),mConStr.IndexOf(';', mConStr.IndexOf("source="))-mConStr.IndexOf("source="));
            oldmConStr = mConStr;
            mConStr=mConStr.Replace(toReplace,"source="+serverName.Split(';')[0]);
            toReplace = mConStr.Substring(mConStr.IndexOf("initial catalog="), mConStr.IndexOf(';', mConStr.IndexOf("initial catalog=")) - mConStr.IndexOf("initial catalog="));
            mConStr = mConStr.Replace(toReplace, "initial catalog=" + serverName.Split(';')[1]);
            //System.Windows.Forms.MessageBox.Show(mConStr);
        }

        public void retConStrToPrev()
        {
            if(oldmConStr.Length>5)  mConStr = oldmConStr;
        }
        ...
        private DateTime eBegin;
        private DateTime eEnd;
            ...
            try
            {
                switch (int.Parse(mSrvParamsType[i].ToString()))
                {
                    ...
                    case 58:
                        ...
                        {
                            sqlParam.Value = System.DateTime.Parse("01/01/1900");
                        }
                    ...
                    //sql_variant
                    case 98:
                        sqlParam = new SqlParameter(mSrvParams[i].ToString(), SqlDbType.Variant);
                        sqlParam.Value = mParams[i];
                        break;
                    default:
        ...
        private void WriteExeLog(double t)
        {
            
            if (t > 30) WriteLog(string.Format("Work time={0}. SP = {1}", t, mCom.CommandText), -48, mCom.CommandText);
        }

        private void WriteLog(string message, int ErrorCode, string Source)
        {
            
            using (SqlConnection con=new SqlConnection(mConStr))
			{
				
				SqlCommand com=new SqlCommand("mAddLog",con);
				
                com.Parameters.Add(new SqlParameter("@LogMessage", SqlDbType.NVarChar));
                com.Parameters["@LogMessage"].Value = message;
                com.Parameters.Add(new SqlParameter("@source", SqlDbType.NVarChar));
                com.Parameters["@source"].Value = Source;
                com.Parameters.Add(new SqlParameter("@Error_Code", SqlDbType.Int));
                com.Parameters["@Error_Code"].Value = ErrorCode;
				com.CommandType=CommandType.StoredProcedure;
        ...
        private double getETimeSec()
        {
            
            return (eEnd - eBegin).TotalSeconds;
        }
